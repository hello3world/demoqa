from pages.base_page import BasePage
from playwright.sync_api import expect


class HomePage(BasePage):
    def __init__(self, page):
        super().__init__(page)

    def page_verify(self):
        # The demoqa landing page sometimes changes its banner image; verify by checking
        # that the main card 'Elements' is visible which indicates the page loaded.
        expect(self.page.get_by_text("Elements", exact=True)).to_be_visible(
            timeout=10000
        )

    def click_elements_button(self):
        self.page.get_by_text("Elements").click()

    def open_forms_button(self):
        self.page.get_by_text("Forms").click()
        self.page.get_by_text("Practice Form").click()

    def click_alerts_button(self):
        self.page.get_by_text("Alerts, Frame & Windows").click()

    def click_widgets_button(self):
        self.page.get_by_text("Widgets").click()

    def click_interactions_button(self):
        self.page.get_by_text("Interactions").click()

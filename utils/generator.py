...
def generated_person() -> Generator[Person, Any, None]:
    """Yield a randomly generated Person model."""
    yield Person(
...
def generated_file() -> tuple[str | Any, str]:
    """Create a small text file under utils/ and return (name, path)."""
    utils_dir = os.path.join(os.getcwd(), 'utils')
    os.makedirs(utils_dir, exist_ok=True)
    path = os.path.join(utils_dir, f'text_file{random.randint(0, 999)}.txt')
...
def generated_color() -> Generator[Color, Any, None]:
    yield Color(
        color_name=[
            'Red', 'Blue', 'Green', 'Yellow', 'Purple', 'Black', 'White', 'Voilet',
            'Indigo', 'Magenta', 'Aqua',
        ]
    )
...
def generated_date() -> Generator[Date, Any, None]:
    yield Date(year=fake_en.year(), month=fake_en.month_name(), day=fake_en.day_of_month(), time='12:00')
